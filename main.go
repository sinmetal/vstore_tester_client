...
func main() {
	for {
		...
		for i := 0; i < 200; i++ {
			...
			if _, err := PostItemOnlyOneClientOtherProject(lot, i); err != nil {
				fmt.Println(err.Error())
			}
		}
		if retryInterval > 10 {
...
func PostItemOnlyOneClientOtherProject(lot string, index int) (key string, err error) {
	log := slog.Start(time.Now())
	defer log.Flush()
	...
	req, err := http.NewRequest(
		"POST",
		fmt.Sprintf("%s/item/onlyoneclientotherProject", vtServerURL),
		strings.NewReader(string(b)),
	)
	req.Header.Set("Content-Type", "application/json; charset=utf-8")
	log.Infof("request.body = %s", string(b))
	...
	lm := struct {
		...
		Resource:           "PostItemOnlyOneClientOtherProject",
		Lot:                lot,
		Index:              index,
		Contents:           contents,
		ResponseStatusCode: res.StatusCode,
